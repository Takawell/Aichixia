...
const PLAN_CFG: Record<string, { gradDark:string; gradLight:string; softDark:string; softLight:string; textDark:string; textLight:string; borderDark:string; borderLight:string; label:string }> = {
  enterprise: { gradDark:'linear-gradient(135deg,#a78bfa,#7c3aed)', gradLight:'linear-gradient(135deg,#8b5cf6,#6d28d9)', softDark:'rgba(139,92,246,0.13)', softLight:'rgba(139,92,246,0.1)', textDark:'#c4b5fd', textLight:'#7c3aed', borderDark:'rgba(139,92,246,0.32)', borderLight:'rgba(139,92,246,0.3)', label:'Enterprise' },
...
function StatCard({ label, value, unit='', sub, icon, gradDark, gradLight, glowDark, glowLight, textDark, textLight, barPct, delay=0, liveMode=false }: {
  ...
  return (
    <div ref={ref} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      style={{
        position:'relative', padding:'13px 14px', overflow:'hidden',
      ...
      <div style={{ position:'relative', zIndex:1 }}>
        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:9 }}>
          <div style={{ width:26, height:26, borderRadius:8, background: grad, display:'flex', alignItems:'center', justifyContent:'center', fontSize:12, color:'#fff', flexShrink:0, boxShadow:`0 3px 10px ${glow}` }}>{icon}</div>
          <p className="text-zinc-500 dark:text-zinc-400" style={{ fontSize:9.5, fontWeight:700, letterSpacing:'0.09em', textTransform:'uppercase' }}>{label}</p>
        </div>
        ...
        {barPct !== undefined && !liveMode && (
          <div className="bg-zinc-100 dark:bg-zinc-800/70" style={{ width:'100%', height:3, borderRadius:99, overflow:'hidden', marginBottom:6 }}>
...
function FilterBar({ timeRange, setTimeRange, selectedModel, setSelectedModel, models, onRefresh, refreshing, dark }: {
  ...
  const ranges: { v:'1h'|'6h'|'24h'; l:string }[] = [{ v:'1h', l:'1 Hour' }, { v:'6h', l:'6 Hours' }, { v:'24h', l:'24 Hours' }];
  return (
    <div style={{ display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:10 }}>
      <div style={{ display:'flex', flexWrap:'wrap', gap:6, alignItems:'center' }}>
        <div className="border border-zinc-100 dark:border-zinc-800/60 bg-white/80 dark:bg-white/[0.025]" style={{ display:'flex', borderRadius:12, padding:3, gap:2, backdropFilter:'blur(18px)' }}>
          {ranges.map(({ v, l }) => {
            const active = timeRange === v;
            return (
              <button key={v} onClick={() => setTimeRange(v)}
                style={{
                  all:'unset', cursor:'pointer', padding:'5px 12px', borderRadius:9, fontSize:11, fontWeight:700,
        ...
        {models.length > 0 && (
          <select value={selectedModel} onChange={e => setSelectedModel(e.target.value)}
            ...
            style={{ padding:'6px 10px', borderRadius:10, fontSize:11, fontWeight:600, outline:'none', cursor:'pointer', backdropFilter:'blur(18px)', appearance:'none', minWidth:110 }}>
      ...
      <button onClick={onRefresh} disabled={refreshing}
        style={{
          all:'unset', cursor: refreshing ? 'not-allowed' : 'pointer', display:'flex', alignItems:'center', gap:6,
        ...
        <FiRefreshCw style={{ fontSize:12, animation: refreshing ? 'mn-spin 0.8s linear infinite' : 'none' }} />
        Refresh
      </button>
    </div>
  );
...
function Panel({ title, icon, iconBg, badge, children, delay=0 }: {
  ...
  return (
    <div ref={ref}
      ...
      style={{ padding:'16px', backdropFilter:'blur(18px)', WebkitBackdropFilter:'blur(18px)', opacity: visible ? 1 : 0, transform: visible ? 'translateY(0)' : 'translateY(18px)', transition:`opacity 0.55s ease ${delay}ms, transform 0.55s ease ${delay}ms` }}>
...
function EmptyState({ icon, message }: { icon: React.ReactNode; message: string }) {
  return (
    <div style={{ textAlign:'center', padding:'28px 0' }}>
      <div className="bg-zinc-100 dark:bg-zinc-800/60" style={{ width:48, height:48, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 10px', fontSize:20 }}>
...
function SkeletonCard() {
  return (
    <div className="rounded-2xl border border-zinc-100 dark:border-zinc-800/60 bg-white/80 dark:bg-white/[0.025]" style={{ padding:'13px 14px', backdropFilter:'blur(18px)' }}>
      <div className="bg-zinc-100 dark:bg-zinc-800" style={{ width:26, height:26, borderRadius:8, marginBottom:9, animation:'mn-shimmer 1.5s infinite' }} />
      ...
      <div className="bg-zinc-100 dark:bg-zinc-800" style={{ width:'60%', height:20, borderRadius:6, animation:'mn-shimmer 1.5s infinite 0.2s' }} />
    </div>
  );
}
  ...
  if (loading) {
    return (
      <>
        ...
        <div className="mn-wrap" style={{ display:'flex', flexDirection:'column', gap:11 }}>
  ...
  return (
    <>
      <style>{`
        ...
        .mn-g4 { display:grid; grid-template-columns:repeat(2,1fr); gap:9px; }
        .mn-g2 { display:grid; grid-template-columns:1fr; gap:9px; }
      ...
      <div className="mn-wrap" style={{ display:'flex', flexDirection:'column', gap:11 }}>
        ...
        <div className="mn-g2">
          <Panel
            ...
            delay={0}
            badge={
              <div style={{ display:'flex', alignItems:'center', gap:6, padding:'4px 10px', borderRadius:8, background: dark ? 'rgba(14,165,233,0.12)' : 'rgba(14,165,233,0.08)', border:`1px solid ${dark ? 'rgba(14,165,233,0.22)' : 'rgba(14,165,233,0.18)'}` }}>
                <span style={{ width:6, height:6, borderRadius:'50%', background:'#0ea5e9', display:'inline-block', animation:'mn-pulse 1.8s infinite' }} />
                <span style={{ fontSize:11, fontWeight:700, color: dark ? '#38bdf8' : '#0284c7' }}>{activeUsers.length} online</span>
              </div>
            }>
            {activeUsers.length === 0
              ? <EmptyState icon={<FiUsers />} message="No active users in this time range" />
              : <div style={{ display:'flex', flexDirection:'column', gap:6 }}>{activeUsers.map((u,i) => <UserRow key={u.user_id} user={u} index={i} />)}</div>
            }
          </Panel>
          ...
          <Panel
            ...
            badge={
              filteredLogs.length > 0 ? (
                <div style={{ display:'flex', alignItems:'center', gap:5, padding:'4px 10px', borderRadius:8, background: dark ? 'rgba(139,92,246,0.12)' : 'rgba(139,92,246,0.08)', border:`1px solid ${dark ? 'rgba(139,92,246,0.22)' : 'rgba(139,92,246,0.18)'}` }}>
                  <span style={{ fontSize:11, fontWeight:700, color: dark ? '#a78bfa' : '#7c3aed' }}>{filteredLogs.length} logs</span>
                </div>
              ) : undefined
            }>
            {filteredLogs.length === 0
              ? <EmptyState icon={<FiActivity />} message="No activity in this time range" />
              : <div style={{ display:'flex', flexDirection:'column', gap:4 }}>{filteredLogs.slice(0,10).map((log,i) => <LogRow key={log.id} log={log} index={i} />)}</div>
            }
          </Panel>
        </div>
