...
function CharTall({ mousePos, isCovered, isPeeking, isError, isSuccess, isTyping }: { mousePos: MousePos; isCovered: boolean; isPeeking: boolean; isError: boolean; isSuccess: boolean; isTyping: boolean }) {
  ...
  return (
    <div ref={ref} className="char-tall" style={{ position: 'absolute', left: 6, bottom: 0 }}>
      <svg width="80" height="168" viewBox="0 0 80 168" style={{ overflow: 'visible', filter: `drop-shadow(0 8px 22px ${col}55)` }}>
        <defs>
        ...
        {!isCovered && !isPeeking && (
          <>
            <Eye cx={25} cy={30} r={9} pr={5} dx={dir.x} dy={dir.y} />
            <Eye cx={55} cy={30} r={9} pr={5} dx={dir.x} dy={dir.y} />
            {isError && <path d="M16 49 Q40 43 64 49" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" />}
            {isSuccess && <path d="M16 46 Q40 53 64 46" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" />}
            {!isError && !isSuccess && <path d={isTyping ? 'M18 48 Q40 42 62 48' : 'M18 46 Q40 51 62 46'} stroke="white" strokeWidth="1.8" fill="none" strokeLinecap="round" opacity="0.6" />}
          </>
        )}
        {isCovered && !isPeeking && (
          <>
            <ClosedEye cx={25} cy={30} r={9} />
            <ClosedEye cx={55} cy={30} r={9} />
            <path d="M18 47 Q40 42 62 47" stroke="white" strokeWidth="1.8" fill="none" strokeLinecap="round" opacity="0.4" />
          </>
        )}
        {isPeeking && (
          <>
            <PeekEye cx={25} cy={32} dx={dir.x * 0.5} r={9} />
            <PeekEye cx={55} cy={32} dx={dir.x * 0.5} r={9} />
          </>
        )}
      </svg>
    </div>
...
function CharRound({ mousePos, isCovered, isPeeking, isError, isSuccess }: { mousePos: MousePos; isCovered: boolean; isPeeking: boolean; isError: boolean; isSuccess: boolean }) {
  ...
  return (
    <div ref={ref} className="char-round" style={{ position: 'absolute', left: 112, bottom: 0 }}>
      <svg width="100" height="100" viewBox="0 0 100 100" style={{ overflow: 'visible', filter: `drop-shadow(0 8px 22px ${col}55)` }}>
        <defs>
        ...
        {!isCovered && !isPeeking && (
          <>
            <Eye cx={34} cy={43} r={8.5} pr={4.8} dx={dir.x} dy={dir.y} />
            <Eye cx={66} cy={43} r={8.5} pr={4.8} dx={dir.x} dy={dir.y} />
            {isError && <path d="M28 63 Q50 56 72 63" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" />}
            {isSuccess && <path d="M28 60 Q50 68 72 60" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" />}
            {!isError && !isSuccess && <path d="M30 60 Q50 67 70 60" stroke="white" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.65" />}
          </>
        )}
        {isCovered && !isPeeking && (
          <>
            <ClosedEye cx={34} cy={43} r={8.5} />
            <ClosedEye cx={66} cy={43} r={8.5} />
            <path d="M30 60 Q50 65 70 60" stroke="white" strokeWidth="1.8" fill="none" strokeLinecap="round" opacity="0.4" />
          </>
        )}
        {isPeeking && (
          <>
            <PeekEye cx={34} cy={46} dx={dir.x * 0.5} r={8.5} />
            <PeekEye cx={66} cy={46} dx={dir.x * 0.5} r={8.5} />
          </>
        )}
      </svg>
    </div>
...
function CharSmall({ mousePos, isCovered }: { mousePos: MousePos; isCovered: boolean }) {
  const ref = useRef<HTMLDivElement>(null);
  const dir = useEyeDir(ref as React.RefObject<HTMLElement>, mousePos, 3);
  return (
    <div ref={ref} className="char-small" style={{ position: 'absolute', left: 230, bottom: 0 }}>
      <svg width="70" height="100" viewBox="0 0 70 100" style={{ overflow: 'visible', filter: 'drop-shadow(0 7px 18px rgba(234,179,8,0.55))' }}>
        ...
        {!isCovered ? (
          <>
            <Eye cx={26} cy={53} r={7} pr={4} dx={dir.x * 0.65} dy={dir.y * 0.65} />
            <Eye cx={44} cy={53} r={7} pr={4} dx={dir.x * 0.65} dy={dir.y * 0.65} />
            <path d="M22 66 Q35 72 48 66" stroke="#78350f" strokeWidth="1.6" fill="none" strokeLinecap="round" opacity="0.5" />
          </>
        ) : (
          <>
            <LookAway cx={26} cy={53} r={7} />
            <LookAway cx={44} cy={53} r={7} />
            <path d="M22 68 Q35 64 48 68" stroke="#78350f" strokeWidth="1.6" fill="none" strokeLinecap="round" opacity="0.45" />
          </>
        )}
...
function CharGhost({ mousePos, isCovered }: { mousePos: MousePos; isCovered: boolean }) {
  const ref = useRef<HTMLDivElement>(null);
  ...
  return (
    <div ref={ref} className="char-ghost" style={{ position: 'absolute', left: 316, bottom: 0 }}>
      <svg width="68" height="112" viewBox="0 0 68 112" style={{ overflow: 'visible', filter: 'drop-shadow(0 7px 20px rgba(20,184,166,0.5))' }}>
        ...
        {!isCovered ? (
          <>
            <Eye cx={23} cy={42} r={8} pr={4.5} dx={dir.x * 0.7} dy={dir.y * 0.7} />
            <Eye cx={45} cy={42} r={8} pr={4.5} dx={dir.x * 0.7} dy={dir.y * 0.7} />
            <path d="M23 57 Q34 63 45 57" stroke="white" strokeWidth="1.8" fill="none" strokeLinecap="round" opacity="0.6" />
          </>
        ) : (
          <>
            <LookAway cx={23} cy={42} r={8} />
            <LookAway cx={45} cy={42} r={8} />
            <path d="M23 59 Q34 55 45 59" stroke="white" strokeWidth="1.8" fill="none" strokeLinecap="round" opacity="0.45" />
          </>
        )}
...
function OAuthBtn({ onClick, loading, icon, label, dark }: { onClick: () => void; loading: boolean; icon: React.ReactNode; label: string; dark: boolean }) {
  ...
  return (
    ...
    >
      {icon}{loading ? 'Redirecting...' : label}
    </button>
  );
}
      ...
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        @media(max-width:768px){
          ...
          .right-panel{padding:48px 20px 36px !important;}
        }
      ...
      <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Plus Jakarta Sans',sans-serif", background: bg, transition: 'background 0.35s ease' }}>
        ...
        <div ref={leftPanelRef} className="desktop-left" style={{ flex: '0 0 50%', background: leftBg, position: 'relative', overflow: 'hidden', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', transition: 'background 0.35s ease', perspective: '900px' }}>
          ...
          <div style={{ position: 'relative', width: 420, height: 200, maxWidth: '88%', transform: `rotateX(${tiltY * 0.45}deg) rotateY(${tiltX * 0.45}deg)`, transition: 'transform 0.15s ease-out', transformStyle: 'preserve-3d' }}>
            ...
            <CharSmall mousePos={mousePos} isCovered={isCovered} />
            <CharGhost mousePos={mousePos} isCovered={isCovered} />
