...
function FloatingHearts({ active }: { active: boolean }) {
  ...
  return (
    <div style={{ position: 'absolute', top: -20, left: '50%', transform: 'translateX(-50%)', pointerEvents: 'none', zIndex: 10 }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{ position: 'absolute', fontSize: 14, left: `${(i - 1) * 18}px`, animation: `heartFloat 0.8s ease-out ${i * 0.12}s forwards`, opacity: 0 }}>❤️</div>
      ))}
    </div>
  );
}
...
function FloatingStars({ active }: { active: boolean }) {
  ...
  return (
    <div style={{ position: 'absolute', top: -16, left: '50%', transform: 'translateX(-50%)', pointerEvents: 'none', zIndex: 10 }}>
      {['⭐', '✨', '🌟'].map((s, i) => (
        <div key={i} style={{ position: 'absolute', fontSize: 12, left: `${(i - 1) * 16}px`, animation: `heartFloat 0.7s ease-out ${i * 0.1}s forwards`, opacity: 0 }}>{s}</div>
      ))}
    </div>
  );
}
...
function CharTall({ mousePos, isCovered, isPeeking, isError, isSuccess, isTyping, errorExpr, successExpr, reaction, onCharClick }: CharProps) {
  ...
  return (
    <div ref={ref} className="char-tall" style={{ position: 'absolute', left: 6, bottom: 0, cursor: 'pointer', userSelect: 'none' }}>
  ...
  return (
    <div style={{ marginTop: -6, marginBottom: 13 }}>
      <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
        {[1,2,3,4,5].map(i => (
          <div key={i} style={{ flex: 1, height: 3, borderRadius: 2, background: i <= strength ? colors[strength] : (dark ? '#1e40af' : '#e2e8f0'), transition: 'background 0.25s ease' }} />
        ))}
      </div>
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Plus Jakarta Sans',sans-serif", background: bg, transition: 'background 0.35s ease' }}>
        ...
        <div className="right-panel" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '56px 32px 36px', background: bg, transition: 'background 0.35s ease', overflowY: 'auto' }}>
          <div className={shaking ? 'shake' : ''} style={{ width: '100%', maxWidth: 360 }}>
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              ...
              <h1 style={{ fontSize: 26, fontWeight: 800, color: txt, letterSpacing: '-0.03em', marginBottom: 6, lineHeight: 1.1, transition: 'color 0.3s' }}>Create Account</h1>
              <p style={{ fontSize: 13, color: sub, fontWeight: 400, transition: 'color 0.3s' }}>Get started with Aichixia API</p>
            </div>

            <div style={{ marginBottom: 16, padding: '11px 14px', background: dark ? 'rgba(14,165,233,0.1)' : '#f0f9ff', border: `1.5px solid ${dark ? 'rgba(14,165,233,0.25)' : '#bae6fd'}`, borderRadius: 11, display: 'flex', alignItems: 'flex-start', gap: 9 }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0, marginTop: 1 }}>
                <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke={A} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <div>
                <p style={{ fontSize: 12, fontWeight: 700, color: dark ? AL : AD, marginBottom: 2 }}>Recommended</p>
                <p style={{ fontSize: 11, color: sub, lineHeight: 1.5 }}>Sign up faster with GitHub or Google for instant access</p>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12, marginTop: 4 }}>
              ...
              <span style={{ fontSize: 11, color: sub, fontWeight: 500, letterSpacing: '0.05em', transition: 'color 0.3s' }}>Or sign up with email</span>
              <div style={{ flex: 1, height: 1, background: divider, transition: 'background 0.3s' }} />
            </div>
            ...
            <form onSubmit={handleRegister}>
              ...
              >
                <button type="button"
                  onMouseEnter={() => { if (!showConfirmPassword) { setIsCovered(false); setIsPeeking(true); } }}
                  onMouseLeave={() => { if (!showConfirmPassword) { setIsPeeking(false); setIsCovered(true); } }}
            ...
            <p style={{ textAlign: 'center', fontSize: 12, color: sub, marginTop: 16, transition: 'color 0.3s' }}>
              ...
              <Link href="/auth/login" style={{ color: A, fontWeight: 700, textDecoration: 'none' }} onMouseOver={e => ((e.target as HTMLElement).style.textDecoration = 'underline')} onMouseOut={e => ((e.target as HTMLElement).style.textDecoration = 'none')}>Sign In</Link>
            </p>
