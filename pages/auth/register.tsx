...
function PasswordToggleBtn({ show, onToggle, dark }: { show: boolean; onToggle: () => void; dark: boolean }) {
  return (
...
function PasswordStrength({ password, dark }: { password: string; dark: boolean }) {
  ...
  const colors = ['', '#ff6b6b', '#ff922b', '#fcc419', '#51cf66', '#20c997'];
      ...
      <div style={{ display: 'flex', gap: 4, marginBottom: 4 }}>
        {[1, 2, 3, 4, 5].map(i => (
          <div key={i} style={{
            ...
            background: i <= strength ? colors[strength] : (dark ? '#3d3a6e' : '#e9ecef'),
...
export default function Register() {
  ...
  useEffect(() => {
    if (!showPassword && !showConfirmPassword) { setIsCovered(true); setIsPeeking(false); }
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', minHeight: '100vh', fontFamily: "'Plus Jakarta Sans',sans-serif", background: bg, transition: 'background 0.35s ease' }}>
        ...
        <div ref={leftPanelRef} className="desktop-left" style={{ flex: '0 0 52%', background: leftBg, position: 'relative', overflow: 'hidden', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', transition: 'background 0.35s ease', perspective: '800px' }}>
          ...
          <div style={{ position: 'relative', width: 490, height: 230, maxWidth: '90%', transform: `rotateX(${tiltY * 0.5}deg) rotateY(${tiltX * 0.5}deg)`, transition: 'transform 0.15s ease-out', transformStyle: 'preserve-3d' }}>
            ...
            <div style={{ position: 'absolute', bottom: -10, left: '2%', right: '2%', height: 22, background: dark ? 'radial-gradient(ellipse,rgba(108,92,231,0.35) 0%,transparent 70%)' : 'radial-gradient(ellipse,rgba(108,92,231,0.28) 0%,transparent 70%)', animation: 'groundPulse 3s ease-in-out infinite', borderRadius: '50%', transition: 'background 0.35s ease' }} />
          </div>
          ...
          <div style={{ marginTop: 36, textAlign: 'center', padding: '0 48px', transform: `translate(${tiltX * 0.8}px, ${tiltY * 0.8}px)`, transition: 'transform 0.15s ease-out' }}>
            <p style={{ fontSize: 16, fontWeight: 600, color: dark ? '#a29bfe' : '#6c5ce7', lineHeight: 1.5, transition: 'color 0.3s' }}>One unified API for 20+ AI models</p>
            <p style={{ fontSize: 13, color: dark ? '#6c6aaa' : '#a29bfe', marginTop: 6, letterSpacing: '0.02em', fontWeight: 500, transition: 'color 0.3s' }}>Fast · Open Source · Ready to use</p>
          </div>
        ...
        <div className="right-panel" style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '60px 32px 40px', background: bg, transition: 'background 0.35s ease', overflowY: 'auto' }}>
          <div className={shaking ? 'shake' : ''} style={{ width: '100%', maxWidth: 380 }}>
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              ...
              <h1 style={{ fontSize: 28, fontWeight: 800, color: txt, letterSpacing: '-0.03em', marginBottom: 8, lineHeight: 1.1, transition: 'color 0.3s' }}>Create Account</h1>
              <p style={{ fontSize: 14, color: sub, fontWeight: 400, transition: 'color 0.3s' }}>Get started with Aichixia API</p>
            </div>

            <div style={{ marginBottom: 20, padding: '12px 16px', background: dark ? 'rgba(108,92,231,0.1)' : '#f3f0ff', border: `1.5px solid ${dark ? 'rgba(108,92,231,0.25)' : '#d0bfff'}`, borderRadius: 12, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0, marginTop: 1 }}>
                <path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z" stroke="#6c5ce7" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <div>
                <p style={{ fontSize: 13, fontWeight: 700, color: dark ? '#a29bfe' : '#5f3dc4', marginBottom: 2 }}>Recommended</p>
                <p style={{ fontSize: 12, color: dark ? '#8b8ab8' : '#7048e8', lineHeight: 1.5 }}>Sign up faster with GitHub or Google for instant access</p>
              </div>
            </div>
            ...
            {success && (
              <div className="pop-in" style={{ marginBottom: 18, padding: '12px 16px', background: dark ? 'rgba(81,207,102,0.1)' : '#f0fff4', border: `1.5px solid ${dark ? 'rgba(81,207,102,0.3)' : '#b2f2bb'}`, borderRadius: 12, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0, marginTop: 1 }}>
                ...
                <span style={{ fontSize: 13, color: dark ? '#69db7c' : '#2f9e44', lineHeight: 1.45 }}>{success}</span>
              </div>
            )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16, marginTop: 4 }}>
              ...
              <span style={{ fontSize: 12, color: sub, fontWeight: 500, letterSpacing: '0.05em', transition: 'color 0.3s' }}>Or sign up with email</span>
              <div style={{ flex: 1, height: 1, background: divider, transition: 'background 0.3s' }} />
            </div>

            <form onSubmit={handleRegister}>
              <InputField type="email" value={email} onChange={v => { setEmail(v); handleTyping(); }} onFocus={() => {}} onBlur={() => {}} placeholder="Email address" required isError={!!error} dark={dark} />

              <InputField
                type={showPassword ? 'text' : 'password'} value={password}
                onChange={v => { setPassword(v); handleTyping(); }}
                onFocus={() => { if (!showPassword && !showConfirmPassword) setIsCovered(true); }}
                onBlur={() => { if (!showPassword && !showConfirmPassword) setIsCovered(false); }}
                placeholder="Password (min. 6 characters)" required isError={!!error} dark={dark}
              >
                <PasswordToggleBtn
                  show={showPassword} dark={dark}
              ...
              <InputField
                ...
                onChange={v => { setConfirmPassword(v); handleTyping(); }}
                onFocus={() => {
                  if (!showPassword && !showConfirmPassword) setIsCovered(true);
                }}
                onBlur={() => {
                  if (!showPassword && !showConfirmPassword) setIsCovered(false);
                }}
              ...
              >
                <PasswordToggleBtn
                  show={showConfirmPassword} dark={dark}
                  onToggle={() => setShowConfirmPassword(!showConfirmPassword)}
                />
              </InputField>

              {confirmPassword.length > 0 && password !== confirmPassword && (
                <p style={{ fontSize: 12, color: '#ff6b6b', marginTop: -10, marginBottom: 12, paddingLeft: 2 }}>Passwords do not match</p>
              )}
              {confirmPassword.length > 0 && password === confirmPassword && confirmPassword.length >= 6 && (
                <p style={{ fontSize: 12, color: '#51cf66', marginTop: -10, marginBottom: 12, paddingLeft: 2 }}>✓ Passwords match</p>
              )}

              <PrimaryBtn loading={loading}>{loading ? 'Creating account...' : 'Create Account'}</PrimaryBtn>
            </form>

            <p style={{ textAlign: 'center', fontSize: 13, color: sub, marginTop: 6, transition: 'color 0.3s' }}>
              Already have an account?{' '}
              <Link href="/auth/login" style={{ color: link, fontWeight: 700, textDecoration: 'none' }}
