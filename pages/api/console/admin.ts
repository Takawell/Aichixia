      ...
      if (type === 'redemptions') {
        ...
        const redemptionsWithEmail = await Promise.all(
          redemptions.map(async (r: any) => {
            const { data: { user: redeemedUser } } = await supabase.auth.admin.getUserById(r.user_id);
            return {
              ...r,
      ...
      if (type === 'users') {
        ...
        const usersWithDetails = await Promise.all(
          userSettings.map(async (setting: any) => {
            const { data: { user: userData } } = await supabase.auth.admin.getUserById(setting.user_id);
