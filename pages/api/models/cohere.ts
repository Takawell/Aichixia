...
import { chatCohere } from "@/lib/cohere";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await chatCohere(hist);
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "cohere" });

  } catch (err: any) {
    console.error("Cohere API error:", err.message);
    return res.status(500).json({ 
      error: "Cohere is currently unavailable",
      details: err.message 
    });
  }
}
