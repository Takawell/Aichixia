...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await chatMimo(hist);
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "mimo" });

  } catch (err: any) {
    console.error("Mimo API error:", err.message);
    return res.status(500).json({ 
      error: "Mimo is currently unavailable",
      details: err.message 
    });
  }
}
