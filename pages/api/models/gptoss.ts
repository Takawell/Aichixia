...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await chatGptOss(hist);
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "gptoss" });

  } catch (err: any) {
    console.error("GPT-OSS API error:", err.message);
    return res.status(500).json({ 
      error: "GPT-OSS is currently unavailable",
      details: err.message 
    });
  }
}
