...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await chatQwen(hist);
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "qwen" });

  } catch (err: any) {
    console.error("Qwen API error:", err.message);
    return res.status(500).json({ 
      error: "Qwen is currently unavailable",
      details: err.message 
    });
  }
}
