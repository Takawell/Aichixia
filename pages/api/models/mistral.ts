...
import { chatMistral } from "@/lib/mistral";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await chatMistral(hist);
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "mistral" });

  } catch (err: any) {
    console.error("Mistral API error:", err.message);
    return res.status(500).json({ 
      error: "Mistral is currently unavailable",
      details: err.message 
    });
  }
}
