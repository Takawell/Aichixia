...
import { chatMinimax } from "@/lib/minimax";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await chatMinimax(hist);
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "minimax" });

  } catch (err: any) {
    console.error("Minimax API error:", err.message);
    return res.status(500).json({ 
      error: "Minimax is currently unavailable",
      details: err.message 
    });
  }
}
