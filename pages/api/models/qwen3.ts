...
import { chatQwenV2 } from "@/lib/qwen3";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const result = await chatQwenV2(hist);
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "qwen3" });

  } catch (err: any) {
    console.error("Qwen3 API error:", err.message);
    return res.status(500).json({ 
      error: "Qwen3 is currently unavailable",
      details: err.message 
    });
  }
}
