...
import { chatDeepSeekV } from "@/lib/deepseek-v";
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const { message, history, persona, enableThinking } = req.body as {
      message: string;
    ...
    const result = await chatDeepSeekV(hist, { enableThinking });
    return res.status(200).json({ type: "ai", reply: result.reply, provider: "deepseek-v" });

  } catch (err: any) {
    console.error("DeepSeek V API error:", err.message);
    return res.status(500).json({ 
      error: "DeepSeek V is currently unavailable",
      details: err.message 
    });
  }
}
