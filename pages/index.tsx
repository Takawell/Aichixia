...
func main() {
  ...
  return (
    <>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="text-left">
                          <div className="flex items-center gap-1.5">
                            ...
                            {selectedModelData?.requiresPro && (
                              <span className="text-[8px] font-black px-1.5 py-0.5 rounded-full bg-gradient-to-r from-amber-400 to-orange-500 text-white leading-none">PRO</span>
                            )}
                            {selectedModelData?.limited && (
                              <span className="text-[8px] font-black px-1.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border border-amber-300 dark:border-amber-700 leading-none">LIMITED</span>
                            )}
                          </div>
      ...
      <section className="py-12 sm:py-16 lg:py-20 px-3 sm:px-4 lg:px-6 relative overflow-hidden" data-animate="axcode">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute inset-0 bg-[linear-gradient(rgba(59,130,246,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.03)_1px,transparent_1px)] bg-[size:48px_48px] dark:bg-[linear-gradient(rgba(59,130,246,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(59,130,246,0.06)_1px,transparent_1px)]" style={{maskImage:'radial-gradient(ellipse 80% 80% at 50% 50%,black 0%,transparent 100%)'}} />
          <div className="absolute -top-32 -left-32 w-96 h-96 rounded-full bg-blue-500/5 dark:bg-blue-500/10 blur-3xl" />
          <div className="absolute -bottom-32 -right-32 w-96 h-96 rounded-full bg-purple-500/5 dark:bg-purple-500/10 blur-3xl" />
        </div>

        <div className="max-w-7xl mx-auto relative">
          <div className={`transition-all duration-700 ${isVisible.axcode ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <div className="rounded-2xl sm:rounded-3xl overflow-hidden border border-zinc-200 dark:border-zinc-800/80 bg-white dark:bg-zinc-950 shadow-2xl">
              <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-blue-500/50 to-transparent" />

              <div className="grid lg:grid-cols-2 gap-0">
                <div className="p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 w-fit mb-5 sm:mb-6">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-blue-500"></span>
                    </span>
                    <span className="text-[10px] sm:text-xs font-bold text-blue-700 dark:text-blue-300 uppercase tracking-widest">Coming Soon</span>
                  </div>

                  <div className="mb-4 sm:mb-5">
                    <h2 className="text-3xl sm:text-4xl lg:text-5xl font-black text-zinc-900 dark:text-white tracking-tight leading-none mb-1">
                      Ax<span className="bg-gradient-to-r from-blue-500 via-cyan-400 to-violet-500 bg-clip-text text-transparent animate-gradient">Code</span>
                    </h2>
                    <p className="text-xs sm:text-sm font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest mt-1">Agentic Coding Intelligence</p>
                  </div>

                  <p className="text-sm sm:text-base text-zinc-600 dark:text-zinc-400 leading-relaxed mb-6 sm:mb-8 max-w-md">
                    The next evolution of AI-powered development. AxCode is a <span className="font-semibold text-zinc-900 dark:text-white">super-intelligent agentic terminal</span> — plans, writes, debugs, and ships code autonomously, end-to-end.
                  </p>

                  <div className="grid grid-cols-2 gap-2.5 sm:gap-3 mb-6 sm:mb-8">
                    {[
                      { icon: FaBolt, label: "Agentic Execution", desc: "Multi-step autonomous tasks", color: "text-yellow-500" },
                      { icon: FaRobot, label: "Super Intelligence", desc: "Frontier-class reasoning", color: "text-blue-500" },
                      { icon: FaCode, label: "Full Tool Access", desc: "Files, shell, browser, APIs", color: "text-purple-500" },
                      { icon: FaStream, label: "MCP Native", desc: "Extend with any service", color: "text-cyan-500" },
                    ].map((item, i) => (
                      <div key={i} className="flex items-start gap-2.5 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-100 dark:border-zinc-800/60 hover:border-zinc-200 dark:hover:border-zinc-700 transition-all duration-200 group hover:shadow-sm">
                        <item.icon className={`w-4 h-4 mt-0.5 flex-shrink-0 ${item.color} group-hover:scale-110 transition-transform duration-200`} />
                        <div className="min-w-0">
                          <p className="text-xs font-bold text-zinc-900 dark:text-white truncate">{item.label}</p>
                          <p className="text-[10px] text-zinc-500 dark:text-zinc-400 truncate">{item.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-2.5">
                    <div className="group relative flex-1">
                      <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-cyan-500 rounded-xl blur opacity-40 group-hover:opacity-70 transition duration-300"></div>
                      <button className="relative w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold text-white bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 rounded-xl shadow-lg transition-all duration-300 hover:scale-[1.02]">
                    ...
                    <button className="flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold text-zinc-700 dark:text-zinc-300 bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-800 rounded-xl transition-all duration-200 hover:scale-[1.02]">
                ...
                <div className="border-t lg:border-t-0 lg:border-l border-zinc-100 dark:border-zinc-800/60 bg-zinc-950 dark:bg-black p-5 sm:p-6 lg:p-8 flex flex-col justify-center rounded-b-2xl sm:rounded-b-3xl lg:rounded-b-none lg:rounded-r-3xl">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <div className="flex-1 h-6 rounded-md bg-white/5 border border-white/8 flex items-center px-3">
                      <span className="text-[10px] text-zinc-500 font-mono">axcode — ~/my-project</span>
                    </div>
                  </div>

                  <div className="font-mono text-[11px] sm:text-xs leading-relaxed space-y-1.5">
                    <div className="flex gap-2">
                      <span className="text-blue-400/60 flex-shrink-0">❯</span>
                      <span className="text-zinc-300">axcode init <span className="text-zinc-500">--agentic</span></span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-cyan-400">◆ AxCode v1.0 — Powered by Aichixia</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-emerald-400">✔ Agentic core ready</span>
                    </div>
                    <div className="mt-3 flex gap-2">
                      <span className="text-blue-400/60 flex-shrink-0">❯</span>
                      <span className="text-zinc-300">axcode <span className="text-white font-semibold">"build a full auth system"</span></span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-yellow-400">⚡ Planning 18 sub-tasks across 6 files...</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-emerald-400">✔ auth/session.ts — Created</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-emerald-400">✔ auth/middleware.ts — Created</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-emerald-400">✔ auth/jwt.ts — Security hardened</span>
                    </div>
                    <div className="flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-emerald-400">✔ tests/ — All 24 tests passing</span>
                    </div>
                    <div className="mt-2 flex gap-2">
                      <span className="text-transparent flex-shrink-0">❯</span>
                      <span className="text-zinc-400">→ Ready to deploy. <span className="text-blue-400">Deploy now?</span> <span className="inline-block w-1.5 h-3.5 bg-blue-400 animate-pulse align-middle ml-0.5"></span></span>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 pt-4 border-t border-white/8 grid grid-cols-3 gap-3">
                    {[
                      ...
                      <div key={i} className="text-center">
                        <div className="text-lg sm:text-xl font-black text-white tracking-tight">{s.val}</div>
