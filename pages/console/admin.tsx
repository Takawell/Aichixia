...
export default function AdminDashboard() {
  ...
  const handleUpdateDisplayName = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) { setNameSaving(false); showToast('Session expired, please refresh', 'error'); return; }
      const res = await fetch('/api/console/admin', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${session.access_token}` },
        body: JSON.stringify({ action: 'update-user-profile', user_id: selectedUser.user_id, display_name: trimmed }),
      });
      if (res.ok) {
        setSelectedUser(prev => (prev ? { ...prev, display_name: trimmed } : prev));
        setUsers(prev => prev.map(u => (u.user_id === selectedUser.user_id ? { ...u, display_name: trimmed } : u)));
        setShowEditNameModal(false);
        setEditNameValue('');
        showToast('Name updated successfully', 'success');
      } else {
        const body = await res.json().catch(() => ({}));
        showToast(body.error || 'Failed to update name', 'error');
      }
    } catch {
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button
                type="button"
                onClick={e => { e.stopPropagation(); closeEditNameModal(); }}
            ...
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={e => { e.stopPropagation(); handleUpdateDisplayName(); }}
                disabled={nameSaving}
