...
if (!CLOUDFLARE_API_KEY || !CLOUDFLARE_ACCOUNT_ID) {
  console.warn("[lib/kimi] Warning: CLOUDFLARE credentials not set in env. Image generation will be disabled.");
}
...
async function executeImageGenerate(
  ...
  try {
    ...
    console.error("[lib/kimi] Cloudflare image generation error:", error);
...
export async function quickChatKimi(
  ...
  const { reply, imageBase64 } = await chatKimi(hist, {
