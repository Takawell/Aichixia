...
const TTS_VOICE_ID = process.env.TTS_VOICE_ID_MIU || "tc_68d49c1e02c83f1fd4cdeaae";
const TTS_MODEL = process.env.TTS_MODEL_MIU || "ssfm-v30";
const TTS_API_URL = "https://api.typecast.ai/v1/text-to-speech";

if (!TTS_API_KEY) {
  console.warn("[lib/miu] Warning: TTS_API_KEY not set in env.");
}
...
export type SupportedLanguage = "eng" | "jpn" | "kor" | "cmn" | "spa" | "vie";
...
export class MiuError extends Error {
...
export class MiuRateLimitError extends MiuError {
  constructor(message: string) {
    super(message);
    this.name = "MiuRateLimitError";
  }
}

export class MiuQuotaError extends MiuError {
  constructor(message: string) {
    super(message);
    this.name = "MiuQuotaError";
  }
}

function validateText(text: string): void {
  if (!text || text.trim().length === 0) {
    throw new MiuError("Text cannot be empty");
  }

  if (text.length > 2000) {
    throw new MiuError("Text exceeds maximum length of 2000 characters");
  }
}

function validateVolume(volume?: number): void {
  if (volume !== undefined && (volume < 0 || volume > 200)) {
    throw new MiuError("Volume must be between 0 and 200");
  }
}

function validatePitch(pitch?: number): void {
  if (pitch !== undefined && (pitch < -12 || pitch > 12)) {
    throw new MiuError("Pitch must be between -12 and 12 semitones");
  }
}

function validateTempo(tempo?: number): void {
  if (tempo !== undefined && (tempo < 0.5 || tempo > 2.0)) {
    throw new MiuError("Tempo must be between 0.5 and 2.0");
  }
}

function validateEmotionIntensity(intensity?: number): void {
  if (intensity !== undefined && (intensity < 0 || intensity > 2)) {
    throw new MiuError("Emotion intensity must be between 0 and 2");
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        throw new MiuRateLimitError("Rate limit exceeded. Please try again later.");
      }
      if (response.status === 402) {
        throw new MiuQuotaError("Monthly credit quota exceeded.");
      }
      if (response.status === 401) {
        throw new MiuError("Invalid API key.");
      }
      if (response.status === 400) {
        const errorData = await response.json().catch(() => ({}));
        throw new MiuError(`Bad request: ${JSON.stringify(errorData)}`);
      }

      const errorText = await response.text();
      throw new MiuError(`API error (${response.status}): ${errorText}`);
    }
