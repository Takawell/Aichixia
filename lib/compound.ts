...
export async function chatCompound(
  ...
  try {
    if (opts?.onProgress) {
      opts.onProgress({ step: 0, text: 'Searching web', stage: 'searching' });
    }
    ...
    if (opts?.onProgress) {
      opts.onProgress({ step: 1, text: 'Analyzing data', stage: 'analyzing' });
    }
    ...
    for await (const chunk of stream) {
      ...
      if (!hasEmittedGenerating && tokenCount > 5) {
        if (opts?.onProgress) {
          opts.onProgress({ step: 2, text: 'Generating response', stage: 'generating' });
        }
    ...
    if (opts?.onProgress) {
      opts.onProgress({ step: 3, text: 'Complete', stage: 'complete' });
    }

    return { 
      reply: reply.trim() || "Hmph! I can't answer that right now... not that I care!" 
    };
