...
if (!TTS_API_KEY) {
  console.warn("[lib/lindsay] Warning: TTS_API_KEY not set in env.");
}
...
export class LindsayError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "LindsayError";
  }
}

export class LindsayRateLimitError extends LindsayError {
  constructor(message: string) {
    super(message);
    this.name = "LindsayRateLimitError";
  }
}

export class LindsayQuotaError extends LindsayError {
  constructor(message: string) {
    super(message);
    this.name = "LindsayQuotaError";
  }
}

function validateText(text: string): void {
  if (!text || text.trim().length === 0) {
    throw new LindsayError("Text cannot be empty");
  }

  if (text.length > 2000) {
    throw new LindsayError("Text exceeds maximum length of 2000 characters");
  }
}

function validateVolume(volume?: number): void {
  if (volume !== undefined && (volume < 0 || volume > 200)) {
    throw new LindsayError("Volume must be between 0 and 200");
  }
}

function validatePitch(pitch?: number): void {
  if (pitch !== undefined && (pitch < -12 || pitch > 12)) {
    throw new LindsayError("Pitch must be between -12 and 12 semitones");
  }
}

function validateTempo(tempo?: number): void {
  if (tempo !== undefined && (tempo < 0.5 || tempo > 2.0)) {
    throw new LindsayError("Tempo must be between 0.5 and 2.0");
  }
}

function validateEmotionIntensity(intensity?: number): void {
  if (intensity !== undefined && (intensity < 0 || intensity > 2)) {
    throw new LindsayError("Emotion intensity must be between 0 and 2");
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        throw new LindsayRateLimitError("Rate limit exceeded. Please try again later.");
      }
      if (response.status === 402) {
        throw new LindsayQuotaError("Monthly credit quota exceeded.");
      }
      if (response.status === 401) {
        throw new LindsayError("Invalid API key.");
      }
      if (response.status === 400) {
        const errorData = await response.json().catch(() => ({}));
        throw new LindsayError(`Bad request: ${JSON.stringify(errorData)}`);
      }

      const errorText = await response.text();
      throw new LindsayError(`API error (${response.status}): ${errorText}`);
    }
    ...
    if (error instanceof LindsayError) {
      throw error;
    }
