...
export class LlamaConfigError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "LlamaConfigError";
  }
}
...
function ensureConfigured() {
  if (!GROQ_API_KEY) {
    throw new LlamaConfigError("Llama is not configured properly. Please contact the admin.");
  }
}
...
export async function chatLlama(
  ...
  try {
    ...
    console.error("[lib/llama] chatLlama error:", error);

    if (error?.status === 429) {
      throw new LlamaRateLimitError(`Llama rate limit exceeded: ${error.message}`);
    }
    if (error?.status === 402 || error?.code === "insufficient_quota") {
      throw new LlamaQuotaError(`Llama quota exceeded: ${error.message}`);
    }
  ...
  return new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        ...
        if (!receivedAny) {
          try {
            const fallback = await chatLlama(history, opts);
            enqueueChunk({ content: fallback.reply });
          } catch (fallbackError: any) {
            console.error("[lib/llama] streamLlama fallback error:", fallbackError);
        ...
        console.error("[lib/llama] streamLlama error:", error);
