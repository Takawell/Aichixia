...
const GROK_MODEL = process.env.GROK_MODEL || "xai/grok-3";

if (!GROK_API_KEY) {
  console.warn("[lib/grok] Warning: GROK_API_KEY not set in env.");
}
...
export async function chatGrok(
  ...
  if (!GROK_API_KEY) {
    throw new Error("GROK_API_KEY not defined in environment variables.");
  }
...
export function buildPersonaSystemGrok(
...
export async function quickChatGrok(
  userMessage: string,
  opts?: {
    persona?: Parameters<typeof buildPersonaSystemGrok>[0];
    history?: ChatMessage[];
    temperature?: number;
    maxTokens?: number;
  }
  ...
  if (opts?.persona) {
    hist.push(buildPersonaSystemGrok(opts.persona));
  } else {
    hist.push(buildPersonaSystemGrok("tsundere"));
  }
