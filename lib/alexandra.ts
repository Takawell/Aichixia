...
const TTS_VOICE_ID = process.env.TTS_VOICE_ID_ALEXANDRA || "kdmDKE6EkgrWrrykO9Qt";
const TTS_MODEL = process.env.TTS_MODEL_ALEXANDRA || "eleven_multilingual_v2";
const TTS_API_URL = "https://api.elevenlabs.io/v1/text-to-speech";

if (!TTS_API_KEY) {
  console.warn("[lib/alexandra] Warning: ELEVENLABS_API_KEY not set in env.");
}

export type AudioFormat = "mp3" | "wav";

export type SupportedLanguage = "ind" | "eng" | "rus" | "cmn";
...
export class AlexandraError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "AlexandraError";
  }
}

export class AlexandraRateLimitError extends AlexandraError {
  constructor(message: string) {
    super(message);
    this.name = "AlexandraRateLimitError";
  }
}

export class AlexandraQuotaError extends AlexandraError {
  constructor(message: string) {
    super(message);
    this.name = "AlexandraQuotaError";
  }
}

function validateText(text: string): void {
  if (!text || text.trim().length === 0) {
    throw new AlexandraError("Text cannot be empty");
  }

  if (text.length > 5000) {
    throw new AlexandraError("Text exceeds maximum length of 5000 characters");
  }
}

function validateStability(stability?: number): void {
  if (stability !== undefined && (stability < 0 || stability > 1)) {
    throw new AlexandraError("Stability must be between 0 and 1");
  }
}
...
function validateSimilarityBoost(similarityBoost?: number): void {
  if (similarityBoost !== undefined && (similarityBoost < 0 || similarityBoost > 1)) {
    throw new AlexandraError("Similarity boost must be between 0 and 1");
  }
}

function validateStyle(style?: number): void {
  if (style !== undefined && (style < 0 || style > 1)) {
    throw new AlexandraError("Style must be between 0 and 1");
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        throw new AlexandraRateLimitError("Rate limit exceeded. Please try again later.");
      }
      if (response.status === 402) {
        throw new AlexandraQuotaError("Monthly credit quota exceeded.");
      }
      if (response.status === 401) {
        throw new AlexandraError("Invalid API key.");
      }
      if (response.status === 422) {
        const errorData = await response.json().catch(() => ({}));
        throw new AlexandraError(`Unprocessable request: ${JSON.stringify(errorData)}`);
      }

      const errorText = await response.text();
      throw new AlexandraError(`API error (${response.status}): ${errorText}`);
    }
    ...
    if (error instanceof AlexandraError) {
      throw error;
    }
