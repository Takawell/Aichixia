...
const TTS_VOICE_ID = process.env.TTS_VOICE_ID_CATHERINE || "tc_63edf3e806ab09dfc7719403";
const TTS_MODEL = process.env.TTS_MODEL_CATHERINE || "ssfm-v30";
const TTS_API_URL = "https://api.typecast.ai/v1/text-to-speech";

if (!TTS_API_KEY) {
  console.warn("[lib/catherine] Warning: TTS_API_KEY not set in env.");
}
...
export class CatherineError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "CatherineError";
  }
}

export class CatherineRateLimitError extends CatherineError {
  constructor(message: string) {
    super(message);
    this.name = "CatherineRateLimitError";
  }
}

export class CatherineQuotaError extends CatherineError {
  constructor(message: string) {
    super(message);
    this.name = "CatherineQuotaError";
  }
}

function validateText(text: string): void {
  if (!text || text.trim().length === 0) {
    throw new CatherineError("Text cannot be empty");
  }

  if (text.length > 2000) {
    throw new CatherineError("Text exceeds maximum length of 2000 characters");
  }
}

function validateVolume(volume?: number): void {
  if (volume !== undefined && (volume < 0 || volume > 200)) {
    throw new CatherineError("Volume must be between 0 and 200");
  }
}

function validatePitch(pitch?: number): void {
  if (pitch !== undefined && (pitch < -12 || pitch > 12)) {
    throw new CatherineError("Pitch must be between -12 and 12 semitones");
  }
}

function validateTempo(tempo?: number): void {
  if (tempo !== undefined && (tempo < 0.5 || tempo > 2.0)) {
    throw new CatherineError("Tempo must be between 0.5 and 2.0");
  }
}

function validateEmotionIntensity(intensity?: number): void {
  if (intensity !== undefined && (intensity < 0 || intensity > 2)) {
    throw new CatherineError("Emotion intensity must be between 0 and 2");
  }
}
  ...
  try {
    ...
    if (!response.ok) {
      if (response.status === 429) {
        throw new CatherineRateLimitError("Rate limit exceeded. Please try again later.");
      }
      if (response.status === 402) {
        throw new CatherineQuotaError("Monthly credit quota exceeded.");
      }
      if (response.status === 401) {
        throw new CatherineError("Invalid API key.");
      }
      if (response.status === 400) {
        const errorData = await response.json().catch(() => ({}));
        throw new CatherineError(`Bad request: ${JSON.stringify(errorData)}`);
      }

      const errorText = await response.text();
      throw new CatherineError(`API error (${response.status}): ${errorText}`);
    }
    ...
    if (error instanceof CatherineError) {
      throw error;
    }
