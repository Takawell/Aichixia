...
const QWEN_MODEL = process.env.QWEN_MODEL || "qwen/qwen3-coder:free";
...
export function buildPersonaSystemQwen(
...
export async function quickChatQwen(
  userMessage: string,
  opts?: {
    persona?: Parameters<typeof buildPersonaSystemQwen>[0];
    history?: ChatMessage[];
    temperature?: number;
    maxTokens?: number;
  }
  ...
  if (opts?.persona) {
    hist.push(buildPersonaSystemQwen(opts.persona));
  } else {
    hist.push(buildPersonaSystemQwen("tsundere"));
  }
